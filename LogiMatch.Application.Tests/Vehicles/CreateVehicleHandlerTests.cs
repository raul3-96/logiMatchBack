...
public class CreateVehicleHandlerTests
{
    ...
    [Fact]
    public async Task Handle_WhenUserDoesNotOwnTransporterProfile_ShouldThrow()
    {
        using var db = TestDbContextFactory.Create();
    ...
    public async Task Handle_WhenLicensePlateAlreadyExists_ShouldThrow()
    {
        ...
        var transporter = CreateTransporter(userId);
        var otherTransporter = CreateTransporter(Guid.NewGuid());
    ...
    private static TransporterProfile CreateTransporter(Guid? userId = null)
    {
        return new TransporterProfile(userId ?? Guid.NewGuid());
    }
